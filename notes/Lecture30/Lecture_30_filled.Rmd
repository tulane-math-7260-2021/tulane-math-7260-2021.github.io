...
```{r}
two.way.anova <- aov(price ~ cut * color, data = diamonds2)
summary(two.way.anova)
car::Anova(two.way.anova, type = 2)
```

#### Is the interaction effect significant at $\alpha=0.05$ level?

No.  The p-value associated with $F$-statistic of $0.519$ with df of $2$ and $1302$ is $0.595$.

#### Is the main-effect `cut` significant at $\alpha=0.05$ level?
You may answer this question in two ways.

  1. Use the F-statistic and associated p-value from the ANOVA table and conclude it's not significant at $\alpha=0.05$ level.

  2. Given the fact that the interaction effect is not significant, you might want to perform another two-way ANOVA without the interaction term.
  
```{r}
  no.interaction.anova <- aov(price ~ cut + color, data = diamonds2)
  summary(no.interaction.anova)
```
  The conclusion is still the same that `cut` shows no significant main-effect at $\alpha=0.05$ level.

#### Is the main-effect `color` significant at $\alpha=0.05$ level?
You may answer this question similarly as the last question.  The main-effect of `color` is not significant at $\alpha = 0.05$ level.
...
- The blue line is the fitted least square line from the simple linear regression of `price` on `carat`.

- The red line is from the locally weighted scatterplot smoother that performs local regression of the data.
The loess line shows the trend of the data.
...
```{r}
two.way.ancova <- aov(price ~ cut * color + carat, data = diamonds2)
car::Anova(two.way.ancova, type = 2)
```

#### Are any of the effects significant now? Why?

After controlling for `carat`, the two main effects are now significant at $\alpha=0.05$ level.
